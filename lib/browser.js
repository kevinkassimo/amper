...
function createBrowser(cap) {
  ...
  if (cap.browserName === 'chrome') {
    ...
    if (cap.args) {
      cap.args.forEach(arg => chromeOptions.addArguments(arg));
    }
    if (cap.mobileEmulation) {
      chromeOptions.setMobileEmulation(cap.mobileEmulation);
    }
  }
  ...
  if (cap.browserName === 'firefox') {
    ...
    if (cap.args) {
      cap.args.forEach(arg => firefoxOptions.addArguments(arg));
    }
  }
  ...
  if (timeoutsConfig && Object.keys(timeoutsConfig).length > 0) {
    thenableWebDriver.manage().setTimeouts(timeoutsConfig);
  }
